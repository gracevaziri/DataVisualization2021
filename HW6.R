...
getwd()
allLoci <- read.vcfR("HW6/WFUrbanAdaptation/extData/allLoci.vcf")
sumStats.noOutliers <- read.delim("HW6/WFUrbanAdaptation/extData/batch_1.sumstats.tsv", sep="\t", header = TRUE)
sumStats.pops <- read.delim("HW6/WFUrbanAdaptation/extData/batch_1.sumstats.pops.csv", sep="\t", header = FALSE)
envData <- read.csv("HW6/WFUrbanAdaptation/extData/envData.csv", header = TRUE)
outlierLoci <- read.csv("HW6/WFUrbanAdaptation/extData/outlierLoci.csv", header = TRUE)

#save(allLoci, sumStats.noOutliers, sumStats.pops, envData, outlierLoci, file = "HW^/data/WFUrbanAdaptation.rda")
...
BiocManager::install("qvalue", version = "3.12")
...
#this is what the authors did for every locus
locus2 <- "71089_22" ## Specify locus of interest
df.plot2 %>%
  filter(ID == locus2) %>%
  ggplot(aes(x = envType, y = freq, fill = gt_GT_alleles)) +
  scale_fill_brewer(palette="Spectral") +
  geom_boxplot() +
  annotate("text", x = 1.0, y = 0.0, label = locus2, fontface = 2, size = 7) +
  ylim(0.0,1.0) +
...
(df.plot3 %>%
    ggplot(aes(x = freq, y = envType, fill = ID)) +
    scale_fill_brewer(palette="Spectral") +
   geom_boxplot()+
    #annotate("text", label= "ID",x = 1.0, y = 0.0,  fontface = 2, size = 7) +
    #ylim(0.0,1.0) +
    ylab("Genotype frequency") + xlab("") +
    theme_bw() +
    facet_wrap(~gt_GT_alleles)+
    theme(panel.border = element_blank(), panel.grid.major = element_blank(), legend.title=element_blank(),
          panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
          text = element_text(size = 22), legend.text=element_text(size=26), legend.key.size = unit(.4, "in")))
